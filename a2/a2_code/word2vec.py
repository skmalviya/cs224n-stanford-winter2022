...
def naiveSoftmaxLossAndGradient(
    ...
    outside_softmax_vec = softmax(np.dot(outsideVectors,centerWordVec))   # Vx1
    loss = - np.log(outside_softmax_vec[outsideWordIdx])
    gradCenterVec = np.sum(outsideVectors * outside_softmax_vec.reshape(outsideVectors.shape[0],1), axis=0) \
        - outsideVectors[outsideWordIdx]    # 1xd
    gradOutsideVecs = outside_softmax_vec.reshape(outsideVectors.shape[0],1) * centerWordVec # w != o & Vxd
    gradOutsideVecs[outsideWordIdx] = (outside_softmax_vec[outsideWordIdx]-1) * centerWordVec # w == o
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    U = np.take(outsideVectors, indices, axis=0)    # (K+1,d)
    UV = np.dot(U,centerWordVec)    # (K+1,1)
    UV[1:] = UV[1:]*-1  # (K+1,1)
    U_sig = sigmoid(UV) # (K+1,1)
    loss = - np.sum(np.log(U_sig))  # (1,)
    U_sig -= 1  # (K+1,1), (sigma(x)-1)
    U[1:] = U[1:]*-1    # (K+1,d)
    gradCenterVec = np.sum( U_sig.reshape(U.shape[0],1)*U, axis=0)  #(1,d) sum(sigma(x)-1)
    #print (gradCenterVec)
    ...
    for i, ind in enumerate(indices):
        if ind==outsideWordIdx:
            gradOutsideVecs[ind] += U_sig[i]*centerWordVec
        else:
            gradOutsideVecs[ind] += -U_sig[i]*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    #print (centerWordVectors[word2Ind[currentCenterWord]])
    for w in outsideWords:
        l, c_g ,o_g = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[w], outsideVectors, dataset)
        loss += l
        gradCenterVecs[word2Ind[currentCenterWord]] += c_g
        gradOutsideVectors += o_g
    ### END YOUR CODE
