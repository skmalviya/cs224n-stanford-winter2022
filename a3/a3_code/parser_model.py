...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        print ("#"*30, "Model", "#"*30)
        print ("First Layer (ixh):", [('feat',n_features, 'emb_size', self.embed_size), hidden_size])
        print ("Second Layer (hxc):", [hidden_size,n_classes])
        print ("#"*67)

        self.embed_to_hidden_weight = torch.nn.Parameter(torch.empty(n_features * self.embed_size, hidden_size))
        self.embed_to_hidden_bias = torch.nn.Parameter((torch.empty(hidden_size)))

        self.dropout = nn.Dropout(p=dropout_prob)

        self.hidden_to_logits_weight = torch.nn.Parameter(torch.empty(hidden_size, n_classes))
        self.hidden_to_logits_bias = torch.nn.Parameter(torch.empty(n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        #print ("#"*30, "Embedding Lookup", "#"*30)
        #print ("Input size (batch,input_features):",w.shape)
        #print ("Embedding shape (Vocab, size): ", self.embeddings.shape)
        x = torch.index_select(self.embeddings, 0, w.flatten())
        x = x.reshape(w.shape[0],w.shape[1]*self.embed_size) # w.shape == batches, input_features; 
        #print ("Embedding lookup (batch,input_features*embed_size)", x.shape)
        #print ("#"*78)
        ### END YOUR CODE
        return x
