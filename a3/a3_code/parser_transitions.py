...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ['ROOT']
        self.buffer = sentence.copy() # shallow copy to avoid Assertion Error: "parse test failed: the input sentence should not be modified" 
        self.dependencies = []
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses.copy() # shallow copy
    while(unfinished_parses):
        # Extract the partial_parses of batch_size
        minibatch_partial_parses = unfinished_parses[:batch_size]

        # Predict the first step transition for each partial_parses in the minibatch
        minibatch_transitions = model.predict(minibatch_partial_parses)

        # Apply the parse_step() on each partial_parse based on the transition predicted by model.predict()
        for partial_parse, transition in zip(minibatch_partial_parses, minibatch_transitions):
            partial_parse.parse_step(transition)

        # remove the completed partial_parses from the unfinished_parses
        unfinished_parses = [
