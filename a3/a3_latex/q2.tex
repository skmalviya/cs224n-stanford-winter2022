...
\begin{parts}
    ...
    \ifans{
    $2n$\\
    	Since, each word needs two transitions: 1) \texttt{SHIFT}, and 2) one of the \texttt{ARCs} (\texttt{LEFT} or \texttt{RIGHT}) before being removed from the stack. And, parser has to perform only one of these two transition operation in each step. Hence, total $2n$ steps are required to parse complete sentence of length $n$.
    }\newline
...
\ifans{    
\begin{itemize}
\item The best UAS on the dev set: 88.92
\item The best UAS on the test set: 89.28
\end{itemize}    
} 
